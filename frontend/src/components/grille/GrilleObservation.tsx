...
function noteColor(note: number | null): string {
  if (note === null) return "text-[var(--c-faint)]";
  if (note >= 8) return "text-[var(--c-success)]";
  if (note >= 5) return "text-[var(--c-warn-ink)]";
  return "text-[var(--c-danger-ink)]";
}
...
export function GrilleObservation({
  ...
  // ===========================================================================
  return (
    <div className="mx-auto max-w-[1400px] px-4 py-6 text-[var(--c-ink)]">
      <style>{`
        @media print {
      ...
      {/* En-tête */}
      <div className="grille-print rounded-2xl border border-[var(--c-primary)]/15 bg-[var(--c-surface)] p-5 shadow-sm">
        <div className="rounded-xl bg-[var(--c-primary)] px-5 py-3 text-center text-white">
      ...
      <div className="no-print mt-4 flex flex-wrap items-center gap-2">
        ...
        <button
          type="button"
          onClick={() => addRows(1)}
          className="inline-flex items-center gap-1.5 rounded-xl border border-[var(--c-border)] bg-[var(--c-surface)] px-3 py-2 text-sm font-semibold text-[var(--c-ink)] transition hover:border-[var(--c-primary)]"
        >
      ...
      {/* Grille */}
      <div className="grille-print mt-4 overflow-x-auto rounded-2xl border border-[var(--c-primary)]/15 bg-[var(--c-surface)] shadow-sm">
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="bg-[var(--c-primary)] text-white">
              <th className="w-10 border border-white/10 px-1 py-2 text-center text-xs">N°</th>
              <th className="min-w-[170px] border border-white/10 px-2 py-2 text-left text-xs">
                NOM PRÉNOM
              </th>
              {TESTS.map((t) => (
                <th
                  key={t.code}
                  className="min-w-[88px] border border-white/10 px-1 py-2 text-center text-[11px] leading-tight"
                >
              ...
              <th className="w-16 border border-white/10 px-1 py-2 text-center text-[11px]">
                TOTAL
                <br />/60
              </th>
              <th className="w-16 border border-white/10 px-1 py-2 text-center text-[11px]">
              ...
              <th className="no-print w-8 border border-white/10" />
            </tr>
          </thead>
          <tbody>
            {rows.map((r, idx) => {
              ...
              return (
                <tr key={r.id} className="even:bg-[var(--c-bg)]">
                  <td className="border border-[var(--c-border)] px-1 py-1 text-center text-[var(--c-muted)]">
                    {idx + 1}
                  </td>
                  <td className="border border-[var(--c-border)] px-1 py-1">
                    <input
                      value={r.nom}
                      onChange={(e) => setNom(r.id, e.target.value)}
                      placeholder="Nom Prénom"
                      className="w-full rounded-md border border-transparent bg-transparent px-1.5 py-1 text-sm font-medium outline-none focus:border-[var(--c-primary)] focus:bg-[var(--c-surface)]"
                    />
                  </td>
                  {TESTS.map((t, k) => (
                    <td key={t.code} className="border border-[var(--c-border)] px-1 py-1 text-center">
                      <input
                        inputMode="decimal"
                        value={r.raws[k]}
                        onChange={(e) => setRaw(r.id, k, e.target.value)}
                        placeholder={t.direct ? "/10" : "brut"}
                        className="w-full rounded-md border border-transparent bg-transparent px-1 py-1 text-center text-sm outline-none focus:border-[var(--c-primary)] focus:bg-[var(--c-surface)]"
                      />
                  ...
                  <td className="border border-[var(--c-border)] px-1 py-1 text-center font-bold">
                    {res.notes.some((n) => n !== null) ? res.total : "—"}
                  </td>
                  <td className="border border-[var(--c-border)] px-1 py-1 text-center font-bold text-[var(--c-primary)]">
                    {res.notes.some((n) => n !== null) ? res.note20.toFixed(1) : "—"}
                  </td>
                  <td className="no-print border border-[var(--c-border)] text-center">
                    <button
                      type="button"
                      onClick={() => removeRow(r.id)}
                      className="rounded p-1 text-[var(--c-faint)] transition hover:text-[var(--c-danger-ink)]"
            ...
            {/* BILAN CLASSE */}
            <tr className="bg-[var(--c-soft)] font-bold">
              <td className="border border-[var(--c-border3)]" />
              <td className="border border-[var(--c-border3)] px-2 py-2 text-[var(--c-primary)]">BILAN CLASSE</td>
              {bilan.moyTests.map((m, k) => (
                <td key={k} className="border border-[var(--c-border3)] px-1 py-2 text-center text-[var(--c-primary)]">
                  {bilan.nbActifs ? m.toFixed(1) : "—"}
                </td>
              ))}
              <td className="border border-[var(--c-border3)] px-1 py-2 text-center text-[var(--c-primary)]">
                {bilan.nbActifs ? bilan.moyTotal.toFixed(1) : "—"}
              </td>
              <td className="border border-[var(--c-border3)] px-1 py-2 text-center text-[var(--c-primary)]">
                {bilan.nbActifs ? bilan.moyNote20.toFixed(1) : "—"}
              </td>
              <td className="no-print border border-[var(--c-border3)]" />
            </tr>
          </tbody>
        </table>
      </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-[var(--c-muted)]">
        {label}
      </span>
      {children}
    </label>
...
function GuideNotation() {
  return (
    <div className="grille-print mt-6 rounded-2xl border border-[var(--c-primary)]/15 bg-[var(--c-surface)] p-5 shadow-sm">
      <h2 className="font-serif text-lg font-bold text-[var(--c-primary)]">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {TESTS.map((t) => (
          <div key={t.code} className="rounded-xl border border-[var(--c-border)] bg-[var(--c-bg)] p-4">
            <h3 className="font-bold text-[var(--c-primary)]">
              {t.code} — {t.nom} <span className="font-normal text-[var(--c-muted)]">({t.unite})</span>
            </h3>
            <p className="mt-1 text-xs leading-relaxed text-[var(--c-muted2)]">{t.description}</p>

            {t.direct ? (
              <table className="mt-3 w-full border-collapse text-xs">
                <thead>
                  <tr className="bg-[var(--c-primary)] text-white">
                    <th className="border border-white/10 px-2 py-1 text-left">Critère</th>
                    <th className="border border-white/10 px-2 py-1 text-center">Points</th>
                  </tr>
                </thead>
                <tbody>
                  {t.criteres?.map((c) => (
                    <tr key={c.libelle}>
                      <td className="border border-[var(--c-border)] px-2 py-1">{c.libelle}</td>
                      <td className="border border-[var(--c-border)] px-2 py-1 text-center font-semibold">
                        {c.points} pts
                      </td>
                    </tr>
                  ))}
                  <tr className="font-bold">
                    <td className="border border-[var(--c-border)] px-2 py-1">TOTAL (critères atteints)</td>
                    <td className="border border-[var(--c-border)] px-2 py-1 text-center">___ / 10</td>
                  </tr>
                </tbody>
              </table>
            ) : (
              <div className="mt-3 overflow-x-auto">
                <table className="w-full border-collapse text-center text-xs">
                  <tbody>
                    <tr className="bg-[var(--c-soft)] font-bold text-[var(--c-primary)]">
                      {Array.from({ length: 11 }, (_, n) => (
                        <td key={n} className="border border-[var(--c-border3)] px-1 py-1">
                          {n}
                        </td>
                      ))}
                    </tr>
                    <tr>
                      {t.seuils.map((s, n) => (
                        <td key={n} className="border border-[var(--c-border)] px-1 py-1">
                          {s === null ? "—" : n === 10 ? `≥${s}` : s}
                        </td>
                      ))}
                    </tr>
      ...
      <div className="mt-4 rounded-xl bg-[var(--c-primary)] px-4 py-3 text-sm text-white">
