...
import { AiError, getUserApiKey, runSummary, type SummaryInput } from "@/lib/server/ai";
...
export async function POST(request: Request) {
  ...
  if (!body) return jsonError("Données manquantes", 400);

  try {
    const text = await runSummary(key, body);
